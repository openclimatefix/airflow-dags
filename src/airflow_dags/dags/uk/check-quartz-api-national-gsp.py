...
base_url = (
    "http://uk-development-uk-national-quartz-api.eu-west-1.elasticbeanstalk.com"
    if env == "development"
    else "http://uk-production-uk-national-quartz-api.eu-west-1.elasticbeanstalk.com"
)
...
def check_national_forecast_quantiles_order(access_token: str) -> None:
    ...
    plevel_10 <= expectedPowerGenerationMegawatts <= plevel_90 (with 50 MW buffer).
    ...
    for i, forecast_value in enumerate(forecast_values):
        ...
        # Allow a 50 MW buffer on the quantiles check
        # Only raise error if expected is not within [plevel_10 - 50, plevel_90 + 50]
        buffer_mw = 50
    ...
    logger.info(
        ...
        "50 MW buffer applied for values >= 100 MW "
        "(plevel_10 - 50 <= expected <= plevel_90 + 50)",
    )
...
def quartz_api_national_gsp_check() -> None:
    """Dag to check API."""
    _ = PythonOperator(
    ...
    if_any_task_failed = PythonOperator(
        ...
        on_success_callback=get_slack_message_callback(
            additional_message_context=(
                "One of the Quartz API checks has failed. "
                "See which ones have failed on airflow, to help debug the issue. "
                "This is part of moving over to the data-platform "
            ),
